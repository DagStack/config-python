...
- `canonical_json_dumps` previously fell through to Python's
  `json.dumps(..., sort_keys=True)`, which sorts dict keys by their
  UTF-32 code points. Per RFC 8785 §3.2.3 the canonical sort order
  is UTF-16 code units — the two orderings coincide for any key
  inside the Basic Multilingual Plane (BMP), but diverge whenever
  one of the keys lies in a supplementary plane (U+10000+). The
  implementation now pre-sorts keys with `s.encode("utf-16-be")` as
  the sort key and passes them through `json.dumps` with
